...
void jtag_add_ir_scan(struct jtag_tap *active, struct scan_field *in_fields, tap_state_t state)
{
	...
	LOG_INFO("================ ir_scan() =============================================="  );
	LOG_INFO("================ num_bit = %d", in_fields->num_bits);
	int i;
	uint64_t value;
	uint64_t vt;
	...
	value=0;
	vt =0;
	if(in_fields->in_value!=NULL)
	{
	for(i=0;i<in_fields->num_bits;i=i+8)
	{
		vt = in_fields->in_value[i/8];
        vt = vt << i;
		value = value | vt;
	}
	}
	LOG_INFO("================ in_value  = %016lx", value);
	value=0;
	for(i=0;i<in_fields->num_bits;i=i+8)
	{

		vt = in_fields->out_value[i/8];
        vt = vt << i;
		value = value | vt;

	}
	if(in_fields->out_value!=NULL){
	LOG_INFO("================ out_value = %016lx", value);
    }
	...
	tap_state_t state)
{
	assert(state != TAP_RESET);
	LOG_INFO("================ dr_scan() =================================================="  );
	LOG_INFO("================ num_bits  = %d", in_fields->num_bits);
